...
# 设置Streamlit应用
st.title("AI旅行规划器")
st.caption("使用AI旅行规划器规划你的下一次冒险，通过GPT-4o自动研究并规划个性化行程")
...
# 从用户获取OpenAI API密钥
openai_api_key = st.text_input("输入OpenAI API密钥以访问GPT-4o", type="password")
...
if openai_api_key and serp_api_key:
    researcher = Agent(
        ...
        description=dedent(
            """\
        你是一名世界级的旅行研究员。给定旅行目的地和用户的旅行天数，
        生成用于查找相关旅行活动和住宿的搜索词列表。
        然后为每个搜索词进行网络搜索，分析结果，并返回10个最相关的结果。
        """
        ),
        instructions=[
            "给定旅行目的地和用户的旅行天数，首先生成3个与该目的地和天数相关的搜索词。",
            "对每个搜索词使用`search_google`并分析结果。",
    ...
    planner = Agent(
        ...
        description=dedent(
            """\
        你是一名高级旅行规划师。给定旅行目的地、用户的旅行天数和研究结果列表，
        你的目标是生成满足用户需求和偏好的行程草案。
        """
        ),
        instructions=[
            "给定旅行目的地、用户的旅行天数和研究结果列表，生成包含建议活动和住宿的行程草案。",
            "确保行程结构合理、信息丰富且具有吸引力。",
            "确保提供有细节且平衡的行程，尽可能引用事实。",
            "记住：行程的质量很重要。",
            "注重清晰度、连贯性和整体质量。",
            "绝不编造事实或抄袭。始终提供适当的出处。",
        ],
    ...
    with col1:
        if st.button("生成行程"):
            with st.spinner("正在研究你的目的地..."):
                # 首先获取研究结果
                research_results = researcher.run(f"研究{destination}的{num_days}天旅行", stream=False)

                # 显示研究进度
                st.write("研究完成")

            with st.spinner("正在创建你的个性化行程..."):
                # 将研究结果传递给规划师
                prompt = f"""
                目的地：{destination}
                行程天数：{num_days}天
